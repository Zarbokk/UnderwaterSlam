        ...
        Eigen::Transform<double,3,Eigen::Affine,Eigen::ColMajor>& T)
{
    ...
    else
    {
        ...
        std::nth_element (scores.begin(), scores.begin()+index, scores.end(), boost::bind(&std::pair<unsigned int, double>::second, boost::placeholders::_1) < boost::bind(&std::pair<unsigned int, double>::second, boost::placeholders::_2));
